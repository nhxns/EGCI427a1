<template>
  <div class="container">
    ...
    <div class="ui centered buttons">
      <button class="ui primary basic button" type="submit" @click.prevent="editToAPI">Save</button>
...
export default {
  ...
  methods: {
    editToAPI(){
      ...
      console.log(newUser)
      axios.post('http://localhost:5000/users/'+this.$route.params.contactID, newUser)
      .then((response)=>{
  ...
  mounted() {
    axios.get('http://localhost:5000/users/'+this.$route.params.contactID)
    .then((response)=>{
